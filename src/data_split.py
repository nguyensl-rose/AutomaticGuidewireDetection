...
# path = os.path.join(os.getcwd(),"data/raw/data-aurora-phase3-TransMatrix25A")
path = "."
...
for i,group_size_p_cycle in enumerate(parts_list):
    ...
    p_train_data = convert_array_to_text_structure(cycle_data[:rat_train+1],path)
    print(int(len(p_train_data))+rat_val)
    ...
    fn_train = write_to_text(os.path.join(target_path_splits,"cycle_"+str(i+1)+"_cam_train.txt"),p_train_data)
    fn_val = write_to_text(os.path.join(target_path_splits,"cycle_"+str(i+1)+"_cam_val.txt"),p_val_data)
    fn_test = write_to_text(os.path.join(target_path_splits,"cycle_"+str(i+1)+"_cam_test.txt"),p_test_data)
